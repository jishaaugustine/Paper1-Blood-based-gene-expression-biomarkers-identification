...
phenoData=pData(GSE57475)
#phenoSub = phenoData[phenoData$`characteristics_ch1` %in% list("status: AD","status: CTL"),c("geo_accession","characteristics_ch1")]
phenoSub=phenoData[phenoData$`description` %in% list("PD","HC"),c("geo_accession","description") ]

esets=exprs(GSE57475)[,match(row.names(phenoSub),colnames(exprs(GSE57475)))]
c1=as.factor(as.numeric(phenoSub["description"]=="PD"))
...
#hgu133plus2SYMBOL, hgu133aSYMBOL,illuminaHumanv3SYMBOL
x<-illuminaHumanv3SYMBOL
...
filtered_symbol=filtered_symbol[rownames(filtered_symbol)%in% refName,]
mergeEsetsZ=as.data.frame(filtered_symbol)
label=c1
sigZ=feature_limma(mergeEsetsZ,label)

save(sigZ,file="sigZ_57475.RData")
